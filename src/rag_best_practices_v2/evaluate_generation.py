...
async def main():
    print("🔍 v2 Generation 성능 평가 시작 (HyDE + Hybrid + Reranking 기반)")
    ...
    # NOTE: 시간이 너무 오래 걸린다면 아래 주석을 풀고 15~20개 정도로 줄여서 먼저 테스트할 수 있습니다.
...
""")
    ...
    # Ragas 0.4.x 평가 포맷에 맞춘 결과 저장 딕셔너리
    eval_dict = {
    ...
    # 4. Ragas 평가 (Answer Relevancy, Faithfulness)
    print("\n⏳ Ragas 내부 evaluate() 호출 (Answer Relevancy / Faithfulness 측정 중, 시간이 소요됩니다)...")
    dataset = Dataset.from_dict(eval_dict)
    
    # Ragas 0.4.x 명시적 LLM 및 Embeddings 래퍼 적용
