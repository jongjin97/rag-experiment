...
from src.light_rag.storage.base import BaseGraphStorage
...
class GraphStorage(BaseGraphStorage):
    def __init__(self):
        ...
        if KNOWLEDGE_GRAPH_PATH.exists():
            ...
            self.graph = nx.read_gexf(KNOWLEDGE_GRAPH_PATH)
            print(f"Graph loaded: {self.graph.number_of_nodes()} nodes, {self.graph.number_of_edges()} edges.")
        else:
            print(f"Warning: Graph file not found at {KNOWLEDGE_GRAPH_PATH}. Initializing empty graph.")
